...
map_popup = uicontrol('Parent',anal_data,'Style','popupmenu','FontSize',10,...
                       'String',{'Condition Parameters','CV map', 'Activation map', 'APD\CaT map', 'New CV map', 'Rise Time', 'Calcium Decay'},...
...
function mapPopUp_callback(~,~)
    ...
    switch get(map_popup,'Value')
        case 1
            ...
            GUI_conditionParameters(map, handles, signal_scrn1, signal_scrn2, signal_scrn3, signal_scrn4, signal_scrn5); 
        case 2
             % New Conduction Velocity Map
            GUI_NewConductionVelocity(map, handles, starttimemap_edit,endtimemap_edit, f);
        case 3
            %activationMap
            GUI_ActivationMap(map, handles, f,starttimemap_edit,endtimemap_edit);
        case 4
            % APD Map
            GUI_ActionPotentialDurationMap(map, handles, f, starttimemap_edit,endtimemap_edit);
        case 5
            % Rise Time Map
...
%             if (handles.linked)
            if handles.bounds(handles.activeScreenNo) == 1
                ...
                for i_cam = 1:4
                    if handles.allCamData(i_cam).isloaded && handles.bounds(i_cam) == 1
                       plot(handles.time(1:handles.allCamData(i_cam).maxFrame), squeeze(handles.allCamData(i_cam).cmosData(j,i,:)),...
